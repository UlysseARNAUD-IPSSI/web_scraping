(module.exports = () => {
    ...
    (async function () {
        ...
        for (let [year, url] of global.datas.years) {
            let exit = false, cursorPage = 1;
            while (!exit) {
                await run(`database/${year}/${cursorPage}`, url, async page => {
                    ...
                    exit = !(await page.$eval('.pagenumbers .current + a', element => {
                        url = element.href.trim();
                        cursorPage++;
                        return true;
                    }, 'Waiting for page links', [{timeout: 1000, waitUntil: 'load'}], [{timeout: 1000}]) ?? false);
